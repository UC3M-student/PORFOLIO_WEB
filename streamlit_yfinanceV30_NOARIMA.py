...
def yahoo_timeseries(tickers: list[str], period: str = "10y", interval: str = "1wk") -> pd.DataFrame:
    try:
...
def markowitz_portfolio(data: pd.DataFrame, risk_aversion_lambda: float = 1.0) -> pd.Series:
    r = data.pct_change().dropna()
    ...
    return pd.Series(res.x if res.success else np.full(n, 1 / n), index=data.columns, name=f"Markowitz (λ={risk_aversion_lambda:.1f})")
...
with st.expander("⚙️ Advanced Settings", expanded=False):
    ...
    with col1:
        period = "10y"  # Fixed to 10 years as per request
        st.write(f"Data Period: Fixed at 10 years")
...
# Debug: Display available strategy names
# st.write("Available strategies in mc_results:", list(mc_results.keys()))
...
with tab_groups[0]:
    ...
    with core_tabs[1]:
        ...
        ).add_params(selection).interactive().properties(height=450, title=alt.Title("Cumulative Returns", subtitle="Performance across strategies over 10 years")
                                                         ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                                      ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
        st.altair_chart(perf_chart, use_container_width=True)

        st.markdown("### Key Performance Metrics by Strategy")
        cols = st.columns(min(len(portfolio_returns_df.columns), 4))
        strategy_colors = {
            'Minimum Variance': '#FFB6C1',
            'Risk Parity': '#87CEEB',
            # Updated to match standardized name
            'Markowitz (λ=1.0)': '#98FB98',
            'Maximum Sharpe': '#FFA07A',
            'Equal Weight': '#DDA0DD',
            'Maximum Diversification': '#B0E0E6',
            'Momentum': '#F0E68C',
            'Minimum Correlation': '#ADD8E6',
            'Inverse Volatility': '#90EE90',
            'Equal Risk Contribution': '#FFB6C1',
            'Market Cap': '#87CEFA',
            'Inverse Beta': '#98FF98',
            'Maximum Return': '#FF6347',
            'Custom Allocation': '#D8BFD8'
        }

        for i, strategy in enumerate(portfolio_returns_df.columns):
            with cols[i % len(cols)]:
                metrics = performance_metrics(portfolio_returns_df[strategy])
                # Check if strategy exists in mc_results
                if strategy in mc_results:
                    final_returns = mc_results[strategy].iloc[-1]
                    median_return = final_returns.median()
                    p5_return = final_returns.quantile(0.05)
                    p95_return = final_returns.quantile(0.95)
                else:
                    median_return = p5_return = p95_return = np.nan
                    # st.warning(
                    #     f"Monte Carlo results not found for strategy: {strategy}")
                color = strategy_colors.get(strategy, '#FFFFFF')
                st.markdown(
                    f"""
                    <div style="background-color: {color}; padding: 15px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                        <h4 style="color: #2b6cb0; text-align: center; margin: 0;">{strategy}</h4>
                        <div style="margin-top: 10px;">
                            <strong>Ann. Return:</strong> {metrics['Ann. Return']:.2%}<br>
                            <strong>Sharpe:</strong> {metrics['Sharpe']:.2f}<br>
                            <strong>Max Drawdown:</strong> {metrics['Max Drawdown']:.2%}<br>
                            <strong>Median Forecast Return:</strong> {median_return:.2%}<br>
                            <strong>5th Percentile Return:</strong> {p5_return:.2%}<br>
                            <strong>95th Percentile Return:</strong> {p95_return:.2%}
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True
    ...
    with core_tabs[2]:
        ...
        ).interactive().properties(height=450, title=alt.Title("Historical Prices", subtitle=f"Weekly closing prices for 10 years")
                                   ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
    ...
    with core_tabs[6]:
        ...
        combined_chart = alt.layer(ef_chart, portfolio_points).properties(
            ...
            title=alt.Title("Efficient Frontier",
                            subtitle="Markowitz optimal portfolios over 10 years")
        ).configure_view(
            stroke=None
        ).configure_axis(
            labelFont="Inter",
...
with tab_groups[1]:
    ...
    with risk_tabs[1]:
        ...
        ).interactive().properties(height=400, title=alt.Title("Rolling Volatility", subtitle="12-week annualized volatility for each ticker over 10 years")
                                   ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
        ...
        ).properties(height=250, title=alt.Title("Equal Weight Portfolio Drawdown", subtitle="Maximum loss from peak over 10 years")
                     ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0")
    ...
    with risk_tabs[2]:
        ...
        ).interactive().properties(height=450, title=alt.Title("Strategy Drawdowns", subtitle="Maximum loss from peak for each strategy over 10 years")
                                   ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
...
with tab_groups[2]:
    ...
    with insight_tabs[0]:
        ...
        ).interactive().properties(height=450, title=alt.Title("Rolling Sharpe Ratios", subtitle="26-week annualized Sharpe ratios over 10 years")
                                   ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
    ...
    with insight_tabs[1]:
        ...
        ).properties(height=400, title=alt.Title("Market Beta", subtitle="CAPM beta relative to S&P 500 over 10 years")
                     ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0")
    ...
    with insight_tabs[2]:
        ...
        ).interactive().properties(height=450, title=alt.Title("Rolling Correlation", subtitle="26-week correlation with S&P 500 over 10 years")
                                   ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
    ...
    with insight_tabs[3]:
        ...
        hist_chart = alt.Chart(returns_long).mark_bar(opacity=0.7).encode(
            ...
            tooltip=["Strategy", alt.Tooltip("count()", title="Freq")]
                                                                       ...
                                                                       title=alt.Title(
                                                                           "Return Distribution", subtitle="Distribution of weekly returns by strategy over 10 years")
                                                                       ).configure_view(stroke=None).configure_axis(labelFont="Inter", titleFont="Inter", titleFontSize=16, titleColor="#2b6cb0"
                                                                                                                    ).configure_legend(labelFont="Inter", titleFont="Inter", titleColor="#2b6cb0")
...
with tab_groups[4]:
    ...
    with doc_tabs[1]:
        ...
        st.markdown("""
        The application provides several metrics to evaluate portfolio performance over a 10-year period:

        - **Total Return**: The overall percentage return of the portfolio over the 10-year period.
        ...
        - **Maximum Drawdown**: The largest peak-to-trough decline in portfolio value, measuring the worst-case loss over 10 years.
        - **Median Forecast Return**: The median cumulative return from Monte Carlo simulations over a 1-year forecast horizon.
        - **5th Percentile Return**: The 5th percentile of cumulative returns from Monte Carlo simulations, representing a low-end forecast.
        - **95th Percentile Return**: The 95th percentile of cumulative returns from Monte Carlo simulations, representing a high-end forecast.
        """)

    with doc_tabs[2]:
        st.markdown("### Risk Metrics")
        st.markdown("""
        Risk metrics provide insights into the portfolio's risk profile over a 10-year period:
    ...
    with doc_tabs[4]:
        ...
        st.markdown("""
        Additional analyses provide deeper insights into portfolio diversification and risk allocation over a 10-year period:
